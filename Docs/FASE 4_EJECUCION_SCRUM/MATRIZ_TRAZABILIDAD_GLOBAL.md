...
| **RF-AI-001** 🆕 | HU-AI-001: Como ADMIN quiero generar imágenes referenciales mediante IA libre | ADMIN puede hacer `POST /admin/generar-imagen-ia` con prompt de producto o categoría; la IA genera la imagen en tiempo real y la comprime a WebP/JPEG liviano (<30KB) para previsualizar y guardar | `AIImageGeneratorService` + `POST /admin/generar-imagen-ia` + `ProductImageUploader` / `CategoryImageUploader` | Unit tests PASSED — enriquecimiento de prompt, respuesta Base64 optimizada, RBAC | ✅ Listo (2026-07-28 · SDD/TDD) |
