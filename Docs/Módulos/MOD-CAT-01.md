...
**RN-CAT-UI-01 (Diseño Responsivo Compacto y Eliminación de Conteos):** Las tarjetas de categorías muestran únicamente la imagen de referencia, el nombre completo de la categoría y la flecha indicador de navegación (`>`). Se omite intencionalmente el contador numérico de productos (`+N productos`) para ofrecer una visualización minimalista, limpia e inmediatamente legible en el primer encuadre visual (above-the-fold) a 100% de zoom y niveles de calibración con `Control +`.
