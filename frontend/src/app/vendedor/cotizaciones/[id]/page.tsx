...
export default function CotizacionDetailPage() {
  ...
  const handleApplyDiscount = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast("Descuento B2B manual aplicado con éxito");
      fetchCotizacion();
    } catch (err: any) {
        ...
        <div className="max-w-2xl mx-auto space-y-6">
          ...
          {!loading && cotizacion && (
            <div className="space-y-4">
              ...
              {user?.role === "ADMIN" && cotizacion.state === "COTIZACION" && (
                <form onSubmit={handleApplyDiscount} className="bg-white rounded-md border border-[var(--alling-border)] p-6 shadow-sm space-y-4">
                  ...
                  <div className="bg-blue-50 border border-blue-200 text-blue-800 p-3 rounded text-xs">
                    💡 Como Administrador, puedes aplicar un descuento de hasta el 30% a esta cotización. El subtotal se recalculará automáticamente y se congelará.
                  </div>
              ...
              {cotizacion.state === "EXPIRADA" && (
                <div className="bg-red-50 border border-red-200 rounded-md p-4 text-sm text-red-700">
                  ⚠️ Esta cotización ha expirado. El cliente deberá generar una nueva.
                </div>
              )}
              {cotizacion.state === "CONFIRMADO" && (
                <div className="bg-green-50 border border-green-200 rounded-md p-4 text-sm text-green-700">
                  ✅ Esta cotización fue convertida en pedido y confirmada.
                </div>
              )}
            </div>
          )}
