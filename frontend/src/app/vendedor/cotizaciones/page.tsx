...
export default function SellerCotizacionesPage() {
      ...
      <SellerLayout>
        <div className="space-y-6">
          <div className="flex items-center justify-between flex-wrap gap-3">
            <h1 className="text-2xl font-bold text-[var(--alling-text)]">Pipeline de Cotizaciones</h1>
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-[var(--alling-border)]">
                  {cotizaciones.length === 0 && (
                    <tr><td colSpan={5} className="px-4 py-8 text-center text-[var(--alling-metadata)]">Sin cotizaciones en esta vista</td></tr>
                  )}
                  {cotizaciones.map((c) => (
                    <tr key={c.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-center">
                        <Link href={`/vendedor/cotizaciones/${c.id}`}
